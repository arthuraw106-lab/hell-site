  ...
  return (
    ...
    >
      <Link href={`/manga/${manga.slug}`} className="group block h-full">
        <article className="neo-card relative h-full rounded-[2rem] transition duration-500 hover:-translate-y-2 hover:shadow-violet">
          ...
          <div className="p-5">
            <h3 className="truncate text-xl font-black transition group-hover:text-hell-gold">
              {manga.title}
            </h3>
            ...
            <div className="mt-5 flex items-center justify-between border-t border-white/10 pt-4 text-xs text-white/42">
              <span className="flex items-center gap-1">
                <Heart size={14} className="text-hell-red" />
                {manga._count?.likes || 0}
              </span>
              <span className="flex items-center gap-1">
                <Bookmark size={14} className="text-hell-gold" />
