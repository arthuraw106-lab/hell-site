...
export function AdminPage() {
 ...
 <Reveal>
 <section className="card relative mb-8 overflow-hidden rounded-[2.5rem] p-7 md:p-10">
 ...
 className={
 active
 ? 'card premium-border w-full rounded-[2rem] p-4 text-right '
 : 'rounded-[2rem] border border-white/10 bg-white/[0.045] p-4 text-right transition hover:bg-white/[0.075]'
 }
 >
 <div className="mb-3 flex items-center justify-between">
 <span className={active ? 'grid h-11 w-11 place-items-center rounded-2xl bg-hell-violet/20 text-hell-light' : 'grid h-11 w-11 place-items-center rounded-2xl bg-white/10 text-white/45'}>
 <Icon size={20} />
 </span>
 {active ? <Check size={18} className="text-hell-light" /> : <ChevronLeft size={18} className="text-white/25" />}
 </div>
...
function MiniStat({ label, value, icon: Icon }: { label: string; value: number; icon: any }) {
 ...
 <Icon className="mb-3 text-hell-light" size={22} />
...
function DashboardTab({ dashboard }: { dashboard?: Dashboard }) {
 ...
 <div key={label} className="card rounded-[2rem] p-6">
 <div className="mb-5 flex items-center justify-between">
 <span className="grid h-12 w-12 place-items-center rounded-2xl bg-hell-violet/15 text-hell-light">
 <Icon size={24} />
 </span>
...
function MangaAdminTab() {
 ...
 {message ? <div className="card rounded-[2rem] p-4 text-sm leading-7 text-violet-100">{message}</div> : null}
...
function TicketsAdminTab() {
 ...
 <Card className="card rounded-[2.5rem] p-5">
 ...
 <Card className="card flex min-h-[720px] flex-col rounded-[2.5rem] p-0">
 {selected ? (
 <>
 ...
 <div className={message.isAdmin ? 'max-w-[82%] rounded-3xl border border-hell-light/20 bg-hell-light/10 p-4' : 'max-w-[82%] rounded-3xl bg-white/[0.07] p-4'}>
