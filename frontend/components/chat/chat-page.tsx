  ...
  return (
    <AppShell>
      <main className="relative z-10 mx-auto grid max-w-7xl gap-5 px-5 py-8 lg:grid-cols-[360px_1fr]">
        <Card className="neo-card flex h-[calc(100vh-8rem)] min-h-[650px] flex-col overflow-hidden rounded-[2.5rem]">
          <div className="border-b border-white/10 p-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <div className="mb-1 flex items-center gap-2 text-hell-cyan">
                ...
                {rooms.map((room) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn('rounded-3xl p-4 text-right transition', active ? 'bg-hell-violet/18 shadow-violet' : 'bg-white/[0.04] hover:bg-white/[0.07]')}
                    >
        ...
        <Card className="neo-card flex h-[calc(100vh-8rem)] min-h-[650px] flex-col overflow-hidden rounded-[2.5rem]">
            ...
            <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
              ...
              <div className="flex gap-2">
                ...
                <div className="hidden items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.045] px-4 py-2 text-sm text-white/55 md:flex">
                  <Bot size={16} className="text-hell-cyan" />
                  RegisBot آماده‌ست
                </div>
              </div>
            </div>
          ...
          {pendingMedia ? (
            <div className="border-t border-white/10 bg-black/20 p-4">
              <div className="flex items-center justify-between gap-3 rounded-3xl border border-hell-cyan/25 bg-hell-cyan/10 p-3">
          ...
          <div className="border-t border-white/10 p-4">
            <div className="flex items-end gap-2">
              ...
              <button type="button" onClick={recording ? stopRecording : startRecording} disabled={uploading} className={recording ? 'grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-red-500 text-white shadow-glow' : 'grid h-12 w-12 shrink-0 place-items-center rounded-2xl border border-white/10 bg-white/[0.055] text-white/60 transition hover:bg-white/10 hover:text-white'}>
              ...
              <button type="button" onClick={sendNow} disabled={sendMutation.isPending || uploading || recording || (!body.trim() && !pendingMedia)} className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-hell-purple to-hell-violet text-white shadow-violet transition hover:scale-[1.03] disabled:opacity-50">
                <Send />
              </button>
            </div>
          </div>
...
function RoomModal({ editMode, roomName, roomAvatar, setRoomName, setRoomAvatar, onClose, onSubmit, loading }: any) {
  return (
    <div className="fixed inset-0 z-[120] grid place-items-center bg-black/70 p-5 backdrop-blur-xl">
      <div className="neo-card w-full max-w-lg rounded-[2.5rem] p-6">
...
function UserProfileModal({ user, onClose }: { user: ChatUser; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-[120] grid place-items-center bg-black/70 p-5 backdrop-blur-xl">
      <div className="neo-card w-full max-w-md rounded-[2.5rem] p-6 text-center">
...
function renderPendingMediaIcon(type: MessageType) {
  ...
  return <div className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-white/10 text-hell-cyan"><Icon size={18} /></div>;
}
...
function MessageBubble({ message, mine, onProfileClick }: { message: ChatMessage; mine: boolean; onProfileClick: () => void }) {
  ...
  return (
    <div className={mine ? 'flex justify-end' : 'flex justify-start'}>
      <div className={cn('max-w-[88%] rounded-[2rem] p-3 shadow-soft md:max-w-[72%]', mine ? 'bg-gradient-to-br from-hell-purple to-hell-violet text-white' : isBot ? 'border border-hell-cyan/25 bg-hell-cyan/10' : 'border border-white/10 bg-white/[0.055]')}>
        <button type="button" onClick={onProfileClick} className="mb-2 flex items-center gap-2 text-right">
          <div className={cn('grid h-9 w-9 place-items-center overflow-hidden rounded-xl', isBot ? 'bg-hell-cyan/20 text-hell-cyan' : 'bg-white/10')}>
        ...
        {message.mediaUrl ? (
          <div className="mb-2 overflow-hidden rounded-2xl border border-white/10 bg-black/25">
            {message.type === 'IMAGE' ? <a href={message.mediaUrl} target="_blank" rel="noreferrer"><img src={message.mediaUrl} alt="chat media" className="max-h-80 w-full object-cover" /></a> :
             message.type === 'AUDIO' ? <div className="p-4"><div className="mb-2 flex items-center gap-2 text-sm font-bold text-hell-cyan"><Mic size={16} /> ویس</div><audio src={message.mediaUrl} controls className="w-full" /></div> :
             message.type === 'VIDEO' ? <video src={message.mediaUrl} controls className="max-h-96 w-full object-contain" /> :
             <a href={message.mediaUrl} target="_blank" rel="noreferrer" className="flex items-center gap-3 p-4 text-sm font-bold hover:bg-white/10"><Icon size={18} className="text-hell-cyan" />دانلود فایل</a>}
          </div>
        ) : null}
